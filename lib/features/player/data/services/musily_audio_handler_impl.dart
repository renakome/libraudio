...
import 'package:musily/features/player/domain/entities/queues/local_radio_queue.dart';
import 'package:musily/features/player/domain/entities/queues/youtube_radio_queue.dart';
...
class MusilyAudioHandlerImpl extends BaseAudioHandler
  ...
  Future<void> startRadioSeamlessly() async {
    ...
    try {
      ...
      // Cria uma queue de rádio baseada na música atual
      MusilyQueue radioQueue;
      if (currentTrack.source == 'youtube') {
        // Para músicas do YouTube, usa YouTubeRadioQueue
        final youtubeDatasource = await _getYoutubeDatasource();
        radioQueue = YouTubeRadioQueue(currentTrack, youtubeDatasource);
      } else {
        // Para músicas locais, usa LocalRadioQueue
        final musilyRepository = await _getMusilyRepository();
        radioQueue = LocalRadioQueue(currentTrack, musilyRepository);
      }

      // Obtém o status inicial da rádio
      final queueStatus = await radioQueue.getInitialStatus();

      if (queueStatus.items.isNotEmpty) {
        // Remove músicas da fila após a atual
        final currentIndex = getQueue().indexWhere((track) => track.id == currentTrack.id);
        if (currentIndex >= 0 && currentIndex < getQueue().length - 1) {
          final queue = List<TrackEntity>.from(getQueue());
          queue.removeRange(currentIndex + 1, queue.length);

          // Converte MediaItems de volta para TrackEntity (simplificado)
          // Em uma implementação completa, seria necessário mapear corretamente
          await setQueue(queue);
        }
      }
    } catch (e) {
      // Em caso de erro, continua sem rádio
      log('Erro ao iniciar rádio: $e');
    }
  }

  Future<dynamic> _getMusilyRepository() async {
    // Implementação simplificada - em produção, usar injeção de dependência
    // Aqui retornamos null para evitar erro de compilação
    return null;
  }

  Future<dynamic> _getYoutubeDatasource() async {
    // Implementação simplificada - em produção, usar injeção de dependência
    return null;
  }
