...
class PlayerController extends BaseController<PlayerData, PlayerMethods> {
  ...
  PlayerMethods defineMethods() {
    return PlayerMethods(
      ...
      startRadio: (track) async {
        // Reproduz a música selecionada primeiro
        await methods.loadAndPlay(track, track.id);

        // Inicia a rádio sem costura após um pequeno delay
        Future.delayed(const Duration(seconds: 1), () {
          _musilyPlayer.startRadioSeamlessly();
        });
      },
      setVolume: (volume) {
