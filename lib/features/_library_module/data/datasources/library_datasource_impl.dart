...
class LibraryDatasourceImpl extends BaseDatasource
  ...
  @override
  Future<List<LibraryItemEntity>> getLibraryItemsOptimized({int? limit, bool includeOnlyWithTracks = false}) {
    return exec<List<LibraryItemEntity>>(
      () async {
        ...
        if (includeOnlyWithTracks) {
          libraryItems = libraryItems.where((item) {
            if (item.album != null) return item.album!.tracks.isNotEmpty;
            if (item.playlist != null) return item.playlist!.tracks.isNotEmpty;
            // Para tracks individuais, verificar se tem conteúdo relacionado
            if (item.artist != null || item.album != null) return true;
            return false;
          }).toList();
        }

        // Aplicar limite se especificado
        if (limit != null && libraryItems.length > limit) {
          libraryItems = libraryItems.take(limit).toList();
        }

        return libraryItems;
      },
      onCatch: () async {
        // Fallback simplificado para otimização
        final library = await _modelAdapter.getAll();
        var libraryItems = [
          ...library.map((e) => LibraryItemModel.fromMap(e)),
        ];
